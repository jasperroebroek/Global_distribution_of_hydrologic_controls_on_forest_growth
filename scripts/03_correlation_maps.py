...
M_p_pet = mp.Map(P_PET, tiles=(10, 10), window_size=15)
M_th = mp.Map(TH, tiles=(10, 10), window_size=15)
M_fapar = mp.Map(FAPAR, tiles=(10, 10), window_size=15)

with mp.Map(CORR_P_PET_FAPAR_15, mode='w', ref_map=P_PET, tiles=(10, 10), window_size=15) as M_corr_p_pet_fapar_15:
    for i in M_p_pet:
        mp.progress_bar((i + 1) / M_p_pet.c_tiles)
        p_pet = M_p_pet[i]
        th = M_th[i]
        fapar = M_fapar[i]
        fapar[th < 3] = np.nan
...
M_corr_p_pet_fapar_15 = mp.Map(CORR_P_PET_FAPAR_15, tiles=(10, 10))
